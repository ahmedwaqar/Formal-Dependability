...
(*------------------------------------------*)
val k_out_n_alt = store_thm("k_out_n_alt",
  ``!p X k n.  
       prob_space p /\    
...
val k_out_n_RBD_v2 = store_thm("k_out_n_RBD_v2",
  ``!p n k X pr.
...
(*----------------------------------------------*)
val series_rbd_indep_k_out_n_rbd = store_thm("series_rbd_indep_k_out_n_rbd",
  ``!p L n k X pr.
       prob_space p /\  
       ~NULL L /\ 
...
(*----------------------------------------------*)
(* val binomial_events_conds_def = Define 
...
*)
(*----------------------------------------------*)
val binomial_event_def = Define 
`binomial_event (X:'a->extreal) a = PREIMAGE X {Normal (&a)}`;

(*----------------------------------------------*)
val binomial_event_list_def = Define 
...
++ RW_TAC std_ss[]);
(*----------------------------------------------*)
val parallel_rbd_indep_k_out_n_rbd_v1 = store_thm("parallel_rbd_indep_k_out_n_rbd_v1",
  ``!p L1 L n k  pr.
       prob_space p /\
       mutual_indep p ((FLAT (K_out_N_struct_list p L1 k n::L))) /\
       in_events p (FLAT L) /\
...
>> (FULL_SIMP_TAC real_ss[exp_dist_def])
++ POP_ORW
++ REAL_ARITH_TAC);
